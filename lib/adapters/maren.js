...
module.exports = class MARENAdapter extends BaseAdapter {
  ...
  async fetchAddressUUID(addressData) {
    ...
    try {
      ...
      const match = respJson.adresser.find(entry => entry.adresse == addrString);
      return match.id;
    }
    catch (error) {
  ...
  async fetchFractionDates(addressData, addressUUID) {
    ...
    try {
      ...
      disposals.plastic = disposals.paper; // Plastic and paper are collected together
      return disposals;
    }
    catch (error) {
