...
module.exports = class NGIRAdapter extends BaseAdapter {
  ...
  async fetchFractionDates(addressData, addressUUID) {
    const fractionMap = {
      "Restavfall og bioavfall": "general",
      "Papir, papp, plast og bioavfall": "paper",
    };
    ...
    try {
      ...
      for (const [apiFrac, ourFrac] of Object.entries(fractionMap)) {
        if (record.fraksjon_dynamisk === apiFrac) {
          pickups[ourFrac] = dateOfThisWeek;
        }
        if (record.fraksjon_dynamisk_neste === apiFrac) {
          pickups[ourFrac] = dateOfNextWeek;
        }
      }
      pickups.plastic = pickups.paper; // Plastic and paper are collected together
      pickups.food = new Date(Math.min(dateOfThisWeek, dateOfNextWeek)); // Food is collected every week
      return pickups;
    }
    catch (error) {
